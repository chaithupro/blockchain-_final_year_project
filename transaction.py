...
class Transaction():
    ...
    def transfer(cls, from_addr, to_addr, amount):
        ...
        unspents = Vout.get_unspent(from_addr)
        print("Unspent Outputs:", unspents)  # Debugging line
        ready_utxo, change = select_outputs_greedy(unspents, amount)
        
        if ready_utxo is None:
            raise ValueError("Insufficient funds or no suitable UTXOs found for the specified amount.")

        vin = ready_utxo
        vout = [Vout(to_addr, amount), Vout(from_addr, change)]
...
def select_outputs_greedy(unspent, min_value):
    ...
    lessers = [utxo for utxo in unspent if utxo.amount < min_value]
    greaters = [utxo for utxo in unspent if utxo.amount >= min_value]
    greaters.sort(key=lambda utxo: utxo.amount)
    ...
    lessers.sort(key=lambda utxo: utxo.amount, reverse=True)
    result, accum = [], 0
    for utxo in lessers:
        result.append(utxo)
        accum += utxo.amount
